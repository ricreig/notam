...
export default function AffectedElements({ notams }: AffectedElementsProps) {
  ...
  const grouped = useMemo(() => {
    ...
    for (const n of (notams ?? [])) {
      const key = (n as any)?.element ?? 'unknown';
  ...
  const elements = ((catalogs as any)?.elements ?? []) as Array<{ id: string; label: string }>;
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {(grouped ?? []).map(([elementId, items]) => {
        ...
        const severityAverage = Math.round(
          ((items ?? []).reduce((sum, it) => sum + (it as any).severity, 0) / (items?.length || 1)) * 100
        ) / 100;
          ...
          <article key={elementId} className="rounded-lg border border-slate-800 bg-slate-950/60 p-4 shadow-inner shadow-slate-900/30">
            ...
            <div className="mb-2 h-2 w-full overflow-hidden rounded bg-slate-800">
              <div
                ...
                style={{ width: `${Math.min(100, Math.max(0, severityAverage))}%` }}
                aria-hidden
              />
            </div>
            <ul className="space-y-2 text-xs text-slate-300">
              {((items ?? []).slice(0, 4)).map((n) => (
                <li key={(n as any).id} className="line-clamp-2">
                  <strong className="font-mono text-slate-200">{(n as any).number}</strong> {(n as any).text}
                </li>
              ))}
            </ul>
          </article>
