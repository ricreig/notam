...
export default function TimeSelector() {
  ...
  const setTimeMode = useDashboardStore((s) => s.setTimeMode);
  const setAbsoluteRange = useDashboardStore((s) => s.setAbsoluteRange);
  const setRelativeHours = useDashboardStore((s) => s.setRelativeHours);
  const setDailyWindow = useDashboardStore((s) => s.setDailyWindow);

  const handleModeChange = (mode: 'absolute' | 'relative' | 'daily') => { setTimeMode(mode); refreshNotams({}); };
  ...
  const handleRelativeChange = (hours: number) => { setRelativeHours(hours); refreshNotams({}); };
    ...
    <div className="flex h-full flex-col gap-2 rounded-lg border border-slate-800 bg-slate-900/80 px-4 py-3 text-sm text-slate-200">
      ...
      {view.timeMode === 'relative' && (
        <div className="flex items-center gap-2 text-xs">
          <label htmlFor={relativeRangeId} className="text-slate-400">Hours</label>
          <input
            id={relativeRangeId}
            type="range"
            min={1}
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <label className="space-y-1" htmlFor={dailyStartId}>
            ...
            <input
              ...
              onChange={(e) => handleDailyChange('start', e.target.value)}
              className="w-full rounded border border-slate-700 bg-slate-800 px-2 py-1 text-slate-100 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500"
            />
          </label>
          <label className="space-y-1" htmlFor={dailyEndId}>
            ...
            <input
              ...
              onChange={(e) => handleDailyChange('end', e.target.value)}
              className="w-full rounded border border-slate-700 bg-slate-800 px-2 py-1 text-slate-100 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500"
            />
          </label>
        </div>
